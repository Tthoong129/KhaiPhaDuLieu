...
def render_tab_phase3(model):
    def predict_cutmix(input_img):
        if model is None:
            return {"Lỗi: Mô hình CutMix chưa được nạp": 0.0}
        if input_img is None:
            return {"Lỗi: Vui lòng tải ảnh lên trước khi phân loại": 0.0}
        
        try:
            # Tiền xử lý ảnh giống như lúc huấn luyện (ResNet18 224x224)
            raw_img = Image.fromarray(input_img).convert('RGB').resize((224, 224))
